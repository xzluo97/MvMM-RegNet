...
class LabelConsistencyLoss(object):
    ...
    def loss(self, target_prob, warped_atlases_prob, prior):
        ...
        if self.method == 'use_mask':
            ...
            mask = tf.cast(utils.compute_mask_from_prob(target_prob), tf.bool) | tf.cast(utils.compute_mask_from_prob(warped_atlases_prob), tf.bool)
...
class MvMMNetLoss(object):
    ...
    def loss_weight(self, probs, weights, prior):
        """
        Compute the MvMM-Net loss function.

        :param probs: list or tensor, of shape [n_batch, *vol_shape, n_subject, n_class]
        :param weights: list or tensor, of shape [n_batch, *vol_shape, n_subject, n_class]
        :param prior: prior probability of shape [1, 1, 1, 1, n_class]
        :return:
        """

        with tf.name_scope('mvmm_net_loss_weight'):
            if isinstance(probs, (tuple, list)):
                probs = tf.concat(probs, -2)
            ...
            prob_product = utils.get_joint_prob(probs) * prior
            # product of weight maps
            weight_product = tf.reduce_prod(utils.get_normalized_prob(weights), axis=-2)
            weight_product = tf.stop_gradient(weight_product)  # ToDo: design better weight that allows gradient passing
    ...
    def loss_mask(self, probs, prior):
        """
        Compute the MvMM loss function.

        :param probs: list or tensor, of shape [n_batch, *vol_shape, n_subject, n_class]
        :param prior: a tensor of shape [1, 1, 1, 1, n_class]
        ...
        with tf.name_scope('mvmm_net_loss_mask'):
            ...
            prob_product = utils.get_joint_prob(probs) * prior
            # compute log-likelihood
            sum_mask = tf.cast(tf.reduce_any(prob_product > self.eps, axis=-1), tf.float32)
            # sum_mask = tf.cast(tf.reduce_any(mask, axis=-1), tf.float32)
            ll = tf.log(tf.clip_by_value(tf.reduce_sum(prob_product, axis=-1), self.eps, 1.))
            # ll = tf.log(tf.clip_by_value(tf.reduce_sum(prob_product, axis=-1), self.eps, 1.))
            return tf.negative(tf.divide(tf.reduce_sum(ll*sum_mask), tf.reduce_sum(sum_mask)+self.eps))
...
class CrossCorrelation(object):
    ...
    def __init__(self, win=7, eps=1e-5, **kwargs):
        self.win = win if isinstance(win, (list, tuple)) else [win, ]*3
        self.eps = eps
        self.kwargs = kwargs
    ...
    def ncc(self, target, source):
        ...
        with tf.name_scope('cross_correlation'):
            ...
            if self.kernel == 'ones':
                ...
                strides = [1, 1, 1, 1, 1]
                T_sum = tf.nn.conv3d(target, kernel, strides, padding='SAME')
                S_sum = tf.nn.conv3d(source, kernel, strides, padding='SAME')
                T2_sum = tf.nn.conv3d(T2, kernel, strides, padding='SAME')
                S2_sum = tf.nn.conv3d(S2, kernel, strides, padding='SAME')
                TS_sum = tf.nn.conv3d(TS, kernel, strides, padding='SAME')
            elif self.kernel == 'gaussian':
                kernel = utils.gauss_kernel1d(self.win[0] // 6)
                T_sum = utils.separable_filter3d(target, kernel)
                S_sum = utils.separable_filter3d(source, kernel)
                T2_sum = utils.separable_filter3d(T2, kernel)
                S2_sum = utils.separable_filter3d(S2, kernel)
                TS_sum = utils.separable_filter3d(TS, kernel)
            else:
                raise NotImplementedError
...
class LocalDisplacementEnergy(object):
    ...
    def compute_displacement_energy(self, ddf, energy_weight):
        with tf.name_scope('displacement_energy'):
            ...
            self._dTdz = self._gradient_txyz(ddf, self._gradient_dz)
            if energy_weight == 0:
            ...
            if energy_weight is not None:
                if self.energy_type == 'bending':
                    ...
                    dTdzz = self._gradient_txyz(self._dTdz, self._gradient_dz)
                    dTdxy = self._gradient_txyz(self._dTdx, self._gradient_dy)
                    dTdyz = self._gradient_txyz(self._dTdy, self._gradient_dz)
                    dTdxz = self._gradient_txyz(self._dTdx, self._gradient_dz)
                    energy = tf.reduce_mean(
                dTdxx ** 2 + dTdyy ** 2 + dTdzz ** 2 + 2 * dTdxy ** 2 + 2 * dTdxz ** 2 + 2 * dTdyz ** 2)

                elif self.energy_type == 'membrane':
                    norms = self._dTdx ** 2 + self._dTdy ** 2 + self._dTdz ** 2
                    energy = tf.reduce_mean(norms)

                elif self.energy_type == 'gradient_l1':
    ...
    def compute_jacobian_determinant(self, ddf):
        ...
        with tf.name_scope('jacobian_determinant'):
            ...
            jacobian_det = tf.subtract((self._dTdx[..., 0] + 1) * (self._dTdy[..., 1] + 1) * (self._dTdz[..., 2] + 1) +
                                       self._dTdx[..., 2] * self._dTdy[..., 0] * self._dTdz[..., 1] +
                                       self._dTdx[..., 1] * self._dTdy[..., 2] * self._dTdz[..., 0],
                                       self._dTdx[..., 2] * (self._dTdy[..., 1] + 1) * self._dTdz[..., 0] +
                                       (self._dTdx[..., 0] + 1) * self._dTdy[..., 2] * self._dTdz[..., 1] +
                                       self._dTdx[..., 1] * self._dTdy[..., 0] * (self._dTdz[..., 2] + 1),
                                       name='jacobian_det')
            return jacobian_det
