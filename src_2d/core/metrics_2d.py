...
class OverlapMetrics(object):
    ...
    def __init__(self, n_class=1, eps=0.1, mode='tf', **kwargs):

        self.n_class = n_class
        self.eps = eps
        self.mode = mode
        self.kwargs = kwargs
        self.one_hot = kwargs.pop('one_hot', True)
    ...
    def averaged_foreground_dice(self, y_true, y_seg):
        ...
        if self.one_hot:
            y_seg = utils_2d.get_segmentation(y_seg, self.mode)
        dice = 0.
        if self.mode == 'tf':
            ...
            dice /= tf.cast(self.n_class-1, dtype=tf.float32)  # [n_batch]
            if self.reduce_mean:
    ...
    def averaged_foreground_jaccard(self, y_true, y_seg):
        ...
        if self.mode == 'tf':
            ...
            jaccard /= tf.cast(self.n_class - 1, dtype=tf.float32)
            if self.reduce_mean:
...
def average_foreground_dice(predictions, labels):
    ...
    for i in range(n):
        ...
        label = utils_2d.crop_to_shape(np.array(labels[i]), pred.shape)
...
def acc_rate(predictions, labels):
    ...
    for i in range(n):
        err = np.hstack((err, (100.0 * np.average(
            np.argmax(predictions[i], -1) == np.argmax(utils_2d.crop_to_shape(labels[i], predictions[i].shape), -1)))))
    return err
...
def auc_score(predictions, labels):
    ...
    for i in range(n):
        ...
        flat_true = np.reshape(utils_2d.crop_to_shape(labels[i], predictions[i].shape), [-1, n_class])
...
def average_surface_distance(predictions, labels, spacing_mm=(1, 1, 1)):
    ...
    for i in range(n):
        ...
        mask_pred = utils_2d.get_segmentation(predictions[i], mode='np').squeeze(0)
